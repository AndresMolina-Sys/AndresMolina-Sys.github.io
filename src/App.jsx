...
function Navbar({ t, language, onLanguageChange }) {
  ...
  return <nav className="site-nav" aria-label={language === 'es' ? 'Navegación principal' : 'Main navigation'}>
    <div className="shell nav-inner"><a className="nav-brand" href="#inicio">Andrés Molina<span>.</span></a><div className="nav-links">{links.map(([label, href]) => <a key={href} href={href}>{label}</a>)}</div><div className="nav-tools"><a className="nav-contact" href={CV} download="CV_AndrésMolina_2026.pdf" title={t.nav.cv}>{t.nav.cv}</a><a className="nav-contact" href="#contacto">{t.nav.contactArrow}</a><button className="language-toggle" type="button" onClick={() => onLanguageChange(language === 'es' ? 'en' : 'es')} aria-pressed={language === 'en'} aria-label={language === 'es' ? t.language.switchToEnglish : t.language.switchToSpanish}>{language === 'es' ? 'EN' : 'ES'}</button><button className="mobile-nav-toggle" type="button" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="mobile-nav" aria-label={open ? t.menu.close : t.menu.open}><Icon name={open ? 'close' : 'menu'} /></button></div></div>
    <div id="mobile-nav" className={`shell mobile-nav ${open ? 'open' : ''}`}>{links.concat([[t.nav.cv, CV], [t.nav.contact, '#contacto']]).map(([label, href]) => href === CV ? <a key={href} href={href} download="CV_AndrésMolina_2026.pdf" onClick={close}>{label}</a> : <a key={href} href={href} onClick={close}>{label}</a>)}</div>
  </nav>;
}
