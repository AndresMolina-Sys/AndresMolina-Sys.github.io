...
function Icon({ name, className = '', label }) {
  ...
  switch (name) {
    ...
    case 'csharp': return <svg {...base} {...accessible} style={{ width: '1.5rem', height: '1.5rem' }}><title>C#</title><path d="M12 1.5 20.66 6.5v10L12 21.5 3.34 16.5v-10L12 1.5Z" fill="currentColor" /><text x="12" y="15.35" fill="#0b0b0b" fontFamily="Arial, Helvetica, sans-serif" fontSize="8.5" fontWeight="900" textAnchor="middle">C#</text></svg>;
...
function Navbar({ t, language, onLanguageChange }) {
  ...
  return <nav className="site-nav" aria-label={language === 'es' ? 'Navegación principal' : 'Main navigation'}>
    <div className="shell nav-inner"><a className="nav-brand" href="#inicio">Andrés Molina<span>.</span></a><div className="nav-links">{links.map(([label, href]) => <a key={href} href={href}>{label}</a>)}</div><div className="nav-tools"><a className="nav-contact" href="#contacto">{t.nav.contactArrow}</a><button className="language-toggle" type="button" onClick={() => onLanguageChange(language === 'es' ? 'en' : 'es')} aria-pressed={language === 'en'} aria-label={language === 'es' ? t.language.switchToEnglish : t.language.switchToSpanish}>{language === 'es' ? 'EN' : 'ES'}</button><button className="mobile-nav-toggle" type="button" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="mobile-nav" aria-label={open ? t.menu.close : t.menu.open}><Icon name={open ? 'close' : 'menu'} /></button></div></div>
    <div id="mobile-nav" className={`shell mobile-nav ${open ? 'open' : ''}`}>{links.concat([[t.nav.contact, '#contacto']]).map(([label, href]) => <a key={href} href={href} onClick={close}>{label}</a>)}</div>
  </nav>;
}
